...
nczfilelist = [f for f in os.listdir(WORKDIR) if f.endswith("dbz.nc")]
ncvfilelist = [f for f in os.listdir(WORKDIR) if f.endswith("radv.nc")]
...
for ctlfile in ctlfilelist:
    ...
    for ncfname, nc1fname in zip(nczfilelist, ncvfilelist):
        ...
        # Parsing sesuai format: YYYYMMDDHHMMSS...
        date_str = basename[0:8]   # contoh: 20250824
        time_str = basename[8:14]  # contoh: 000425
        strdate = f"{date_str[0:4]}_{date_str[4:6]}_{date_str[6:8]}_{time_str[0:2]}_{time_str[2:4]}"
